...
async def explore_flow(gid: int, uid: int, display_name: str, send):
    ...
    if p.energy < 10:
        await send(error_embed("Exhausted. Rest or use Choco Milk."))
        return
    ...
    if kind == "nothing":
        await send(embed("Quiet streets", "Nothing happens. The city watches."))
        return
    if kind == "money":
        amt = random.randint(50, 250)
        await add_money(gid, uid, amt, "explore")
        await send(embed("Found cash", f"+{fmt_money(amt)}"))
        return
    if kind == "item":
        from database.repositories.inventory import add_item
        await add_item(gid, uid, "milk", 1)
        await send(embed("Convenience store", "Found Choco Milk x1 (`c!inventory`)"))
        return
    if kind == "trainer":
        await send(embed("A coach watches",
            "A trainer is nearby. Use `/trainers` or `c!trainers` to see who can train you."))
        return
    if kind == "boss":
    ...
    if img_path:
        await send(e, view=view, file=discord.File(img_path, filename="card.png"))
    else:
        await send(e, view=view)
...
class Exploration(commands.Cog):
    ...
    async def explore(self, interaction: discord.Interaction):
        ...
        async def send(e, view=None, file=None):
            if file:
                await interaction.followup.send(embed=e, file=file, view=view)
            else:
                await interaction.followup.send(embed=e, view=view)

        await explore_flow(interaction.guild_id, interaction.user.id,
                           interaction.user.display_name, send)

    @commands.command(name="explore", aliases=["e", "patrol"])
    @commands.cooldown(1, 8, commands.BucketType.user)
    async def explore_prefix(self, ctx: commands.Context):
        async with ctx.typing():
            ...
            await explore_flow(ctx.guild.id, ctx.author.id, ctx.author.display_name, send)
