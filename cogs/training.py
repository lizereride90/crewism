...
class Training(commands.Cog):
    ...
    @commands.command(name="trainers", aliases=["coaches"])
    async def trainers_prefix(self, ctx: commands.Context):
        async with SessionLocal() as s:
            r = await s.execute(select(Trainer))
            rows = r.scalars().all()
        txt = "\n".join(f"{t.id}: {t.name} [{t.specialty}] {t.price} Won LV{t.req_level}+" for t in rows)
        await ctx.send(embed=embed("Trainers", txt))

    @commands.command(name="train")
    async def train_prefix(self, ctx: commands.Context, instance_id: int, trainer_id: str):
        gid, uid = ctx.guild.id, ctx.author.id
        p = await repo.get_or_create_player(gid, uid, ctx.author.display_name)
        async with SessionLocal() as s:
            inst = await s.get(CharacterInstance, instance_id)
            if not inst or inst.guild_id != gid or inst.owner_id != uid:
                await ctx.send(embed=error_embed("Not your fighter."))
                return
            tr = await s.get(Trainer, trainer_id)
            if not tr:
                await ctx.send(embed=error_embed("Unknown trainer."))
                return
            if p.level < tr.req_level:
                await ctx.send(embed=error_embed(f"Needs player LV{tr.req_level}."))
                return
            r = await s.execute(select(TrainingSession).where(
                TrainingSession.guild_id == gid, TrainingSession.user_id == uid, TrainingSession.claimed == 0))
            if r.scalars().first():
                await ctx.send("Finish/claim your current session first (`c!claim_train`).")
                return
        try:
            await add_money(gid, uid, -tr.price, "training", ref=trainer_id)
        except ValueError:
        ...
        async with SessionLocal() as s:
            s.add(TrainingSession(guild_id=gid, user_id=uid, instance_id=instance_id,
                                  trainer_id=trainer_id, ends_at=datetime.utcnow() + timedelta(minutes=mins)))
            await s.commit()
        await ctx.send(f"Training started with {tr.name} ({mins}m). Use `c!claim_train` after.")

    @commands.command(name="claim_train", aliases=["claimtrain"])
    async def claim_train_prefix(self, ctx: commands.Context):
        gid, uid = ctx.guild.id, ctx.author.id
        async with SessionLocal() as s:
            r = await s.execute(select(TrainingSession).where(
                TrainingSession.guild_id == gid, TrainingSession.user_id == uid, TrainingSession.claimed == 0))
            ...
            if sess.ends_at > datetime.utcnow():
                left = int((sess.ends_at - datetime.utcnow()).total_seconds() // 60) + 1
                await ctx.send(f"Not done yet (~{left}m left).")
                return
            tr = await s.get(Trainer, sess.trainer_id)
            inst = await s.get(CharacterInstance, sess.instance_id)
            gains = training_gains(tr.specialty if tr else "strength")
            ...
            sess.claimed = 1
            await s.commit()
        await ctx.send(f"Gains: +{gains}. Fighter #{inst.id} improved.")

    @commands.command(name="mastery")
    async def mastery_prefix(self, ctx: commands.Context):
        p = await repo.get_or_create_player(ctx.guild.id, ctx.author.id, ctx.author.display_name)
        rows = []
        for m, stat in (("strength", p.p_str), ("speed", p.p_spd), ("endurance", p.p_end), ("technique", p.p_tech)):
            rows.append(f"{m}: {mastery_check(stat, p.wins)} (stat {stat}, wins {p.wins})")
        await ctx.send(embed=embed("Mastery", "\n".join(rows)))

    @commands.command(name="bloodline", aliases=["bl"])
    async def bloodline_prefix(self, ctx: commands.Context, action: str = "view", bloodline_id: str = "yamazaki"):
        gid, uid = ctx.guild.id, ctx.author.id
        async with SessionLocal() as s:
            bl = await s.get(Bloodline, bloodline_id)
            if not bl:
                await ctx.send(embed=error_embed("Unknown bloodline."))
                return
            stages = (await s.execute(select(BloodlineStage).where(
                BloodlineStage.bloodline_id == bloodline_id).order_by(BloodlineStage.stage))).scalars().all()
            prog = (await s.execute(select(BloodlineProgress).where(
        ...
        if action == "view":
            txt = f"{bl.name}: stage {cur}/{len(stages)}\n" + "\n".join(
                f"{'✅' if x.stage <= cur else '🔒'} S{x.stage} LV{x.level_req}+ {x.name}" for x in stages)
            await ctx.send(embed=embed("Bloodline", txt))
            return
        nxt = bloodline_available([{"stage": x.stage, "level_req": x.level_req} for x in stages], p.level, cur)
        if not nxt:
            await ctx.send("No awakening available (need higher level).")
            return
        async with SessionLocal() as s:
            if prog:
                prog.stage += 1
                ...
                if not row.bloodline_id:
                    row.bloodline_id = bloodline_id
            await s.commit()
        await ctx.send(f"Awakened {bl.name} stage {cur+1}!")
